...
class VerdexSplash(ctk.CTkToplevel):
    ...
    def __init__(self, parent, on_complete):
        ...
        try:
            ...
            vid_path = "assets/verdex_logo_cinematic.mp4"
            if getattr(sys, 'frozen', False):
                base_path = getattr(sys, '_MEIPASS', os.path.dirname(sys.executable))
                vid_path = os.path.join(base_path, "assets", "verdex_logo_cinematic.mp4")
                if not os.path.exists(vid_path):
                    vid_path = os.path.join(os.path.dirname(sys.executable), "assets", "verdex_logo_cinematic.mp4")
            ...
            print("Failed to initialize OpenCV video:", e)
        ...
        if self.cap is not None:
            ...
            if ret:
                try:
                    ...
                    # Resize to fit neatly
                    im = im.resize((380, 214), Image.Resampling.LANCZOS)
                    photo = ImageTk.PhotoImage(im)
                    self.anim_lbl.configure(image=photo)
                    self.anim_lbl.image = photo
                except Exception as e:
                    pass
            else:
                # Loop video if needed, but since it's 3.5s we can just hold last frame
                pass

        if elapsed < 3.5:
            # Continue animating (~30 fps)
