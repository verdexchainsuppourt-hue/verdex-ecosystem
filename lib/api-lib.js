...
async function verifyAdmin(req) {
  ...
  const parseAdminEmails = (value) => {
    ...
    try {
      ...
      // Environment configuration may use a comma-separated list.
      return String(value)
        .split(',')
        .map((email) => email.trim().toLowerCase())
        .filter(Boolean);
  ...
  // Environment configuration is the immutable production bootstrap. The
  // database allow-list is retained only for the existing admin workflow.
  const configuredEmails = parseAdminEmails(
    process.env.VERDEX_ADMIN_EMAILS || process.env.ADMIN_EMAILS
  );
  if (configuredEmails.length > 0) {
    return configuredEmails.includes(normalizedEmail) ? user : null;
  }
  ...
  return databaseEmails.includes(normalizedEmail) ? user : null;
}
