...
def upload_file(local_path, repo_path):
    try:
        ...
        print(f'SKIP (binary): {local_path}')
        return
        
    sha = get_sha(REPO, repo_path)
    b64_content = base64.b64encode(content.encode('utf-8')).decode('utf-8')
    payload = {'message': f'Upload missing {repo_path}', 'content': b64_content}
    if sha:
        payload['sha'] = sha
...
for root, _, files in os.walk('verdex-chain/src'):
    for file in files:
        if file in ['blockchain.js', 'config.js', 'crypto.js']:
            continue # already uploaded or handled
