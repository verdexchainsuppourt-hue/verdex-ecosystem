...
ipcMain.on('window-min', (event) => {
  ...
  const win = BrowserWindow.getFocusedWindow() || BrowserWindow.fromWebContents(event.sender) || authWindow || mainWindow;
  if (win) {
    console.log('IPC: Minimizing window');
    win.minimize();
  } else {
    console.warn('IPC: No window found to minimize');
  }
});
...
ipcMain.on('window-hide-to-tray', (event) => {
  console.log('IPC: window-hide-to-tray received');
  const win = BrowserWindow.getFocusedWindow() || BrowserWindow.fromWebContents(event.sender) || authWindow || mainWindow;
